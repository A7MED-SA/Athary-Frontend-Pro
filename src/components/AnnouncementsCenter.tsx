...
export default function AnnouncementsCenter({ onTriggerToast }: AnnouncementsCenterProps) {
  ...
  return (
    <div className="bg-stone-50 rounded-3xl border border-stone-250 p-4 sm:p-6 text-right font-sans" dir="rtl" id="announcements-management-center">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border border-amber-200/60 p-5 space-y-4 shadow-3xs h-fit">
          ...
          <form onSubmit={handleCreateAnnouncement} className="space-y-4">
            ...
            <div className="space-y-1">
              <label className="text-xs font-bold text-stone-750 block">عنوان التنويه أو الإعلان</label>
              <input 
                type="text" 
                required
                value={formTitle}
            ...
            <div className="space-y-1">
              <label className="text-xs font-bold text-stone-750 block">الجمهور والدارسين المستهدفين بالبلاغ</label>
              <select
                value={formTarget}
            ...
            {/* Content Textarea */}
            <div className="space-y-1">
              <label className="text-xs font-bold text-stone-750 block">مضمون البث والنص التفصيلي</label>
              <textarea 
                required
                rows={5}
                value={formContent}
            ...
            <button
              ...
              className={`w-full bg-orange-700 hover:bg-orange-850 text-white font-black py-3 px-5 rounded-xl text-xs sm:text-sm transition shadow-md border-0 cursor-pointer flex items-center justify-center gap-2 ${
            ...
            return (
              <div className="space-y-4">
                {filtered.map((ann) => (
                  <div 
                    key={ann.id} 
                    className={`bg-white border rounded-2xl p-4 sm:p-5 shadow-3xs hover:border-amber-250 transition-all ${
