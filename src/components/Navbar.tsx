...
export default function Navbar({
  ...
  const navLinks = [
    { label: 'الرئيسية', view: 'landing' as ViewType },
    { label: 'تصفح الدورات', view: 'catalog' as ViewType },
    { label: 'من نحن واتصل بنا 🗺️', view: 'about-contact' as ViewType },
    { label: 'بوابة الطالب', view: 'dashboard' as ViewType },
    { label: 'كابينة المعلّم 🎓', view: 'instructor-dashboard' as ViewType },
    { label: 'لوحة المشرف 👑', view: 'admin-dashboard' as ViewType },
  ];
  ...
  return (
    <nav className="sticky top-0 z-50 bg-amber-50/90 backdrop-blur-md border-b border-amber-100 shadow-sm" id="athary-navbar">
        ...
        <div className="flex justify-between h-20">
          ...
          <div className="flex items-center">
            <button 
              ...
              id="brand-logo-btn"
            >
              {/* Logo: Beautiful Islamic Arch Book SVG representation */}
              <div className="relative w-12 h-12 bg-orange-700 rounded-xl flex items-center justify-center text-amber-100 shadow-md transform transition-all duration-300 group-hover:scale-105 group-hover:bg-orange-850">
                {/* Visual book pages + arch */}
          ...
          {/* Left: Cart, Authentication controls, actions */}
              ...
              {isLoggedIn ? (
                <div className="flex items-center gap-2" id="verified-user-menu">
                  <button
                    ...
                    className="flex items-center gap-2 bg-orange-55 shadow-sm border border-orange-100 hover:bg-orange-100/70 px-4 py-2 rounded-xl transition-all duration-200 text-stone-800"
                  >
                ...
                <div className="flex items-center gap-3" id="guest-menu">
                  ...
                  <button
                    onClick={() => { setActiveView('auth'); }}
                    className="bg-orange-700 hover:bg-orange-850 text-amber-50 font-semibold text-sm px-5 py-2.5 rounded-xl transition-all shadow-md hover:shadow-lg focus:outline-none"
                    id="guest-start-btn"
                  >
                    ابدأ مجاناً
                  </button>
      ...
      {/* Slide-down mobile navigation list */}
      {mobileMenuOpen && (
          ...
          {navLinks.map((link) => (
            <button
              ...
              className={`block w-full text-right px-4 py-3 rounded-xl font-medium text-sm transition-all ${
                activeView === link.view
                  ? 'bg-orange-50 text-orange-750 font-bold border-r-4 border-orange-700'
            ...
            {isLoggedIn ? (
              <div className="space-y-2">
                ...
                <button
                  ...
                  className="w-full text-center bg-orange-100 hover:bg-orange-200 text-orange-850 font-semibold py-3 rounded-xl text-sm transition-all"
                >
              ...
              <>
                ...
                <button
                  onClick={() => { setActiveView('auth'); setMobileMenuOpen(false); }}
                  className="w-full text-center bg-orange-700 hover:bg-orange-850 text-amber-50 py-3 rounded-xl font-semibold text-sm transition-all shadow-md"
                >
                  البدء الآن مجاناً
                </button>
              </>
