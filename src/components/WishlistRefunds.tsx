          ...
          <button
            ...
            className={`px-4 py-2.5 rounded-xl text-xs font-black transition border-0 cursor-pointer flex items-center gap-2 ${
              activeTab === 'wishlist' 
                ...
                : 'bg-white hover:bg-stone-100 text-stone-605 border border-stone-200'
            }`}
          >
      ...
      {activeTab === 'wishlist' && (
        <div className="space-y-4">
          <div>
            ...
            <p className="text-[11px] text-stone-550 mt-1">تجد هنا المناهج والمحاضرات التي تعتزم حجز مقعدك بها لاحقاً بمجالس آثاري الحرة.</p>
          </div>

          {wishlistItems.length === 0 ? (
            <div className="bg-white border rounded-2xl p-16 text-center space-y-4 border-dashed border-amber-250">
              ...
              <button
                ...
                className="bg-orange-700 hover:bg-orange-850 text-white font-black px-6 py-2.5 rounded-xl text-xs transition border-0 cursor-pointer text-center"
              >
                  ...
                  >
                    <div className="flex gap-4 items-center">
                      ...
                      <div className="space-y-1 min-w-0">
                        <span className="text-[9px] bg-orange-100 text-orange-755 font-bold px-2 py-0.5 rounded">
                          {course.category}
                        </span>
                        ...
                        <div className="flex items-center gap-1.5 text-[10px] text-stone-500">
                          ...
                          <span>•</span>
                          <span className="font-mono text-orange-855 font-black">{course.price === 0 ? 'مجاني' : `${course.price} ر.س`}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-2 shrink-0">
                      <button
                        ...
                        className="bg-orange-700 hover:bg-orange-850 text-white text-[10px] font-black py-2 px-3 rounded-lg border-0 cursor-pointer shadow-3xs flex items-center justify-center gap-1.5 whitespace-nowrap"
                      >
                    ...
                    {isPending && (
                      <div className="absolute inset-0 bg-white/70 flex items-center justify-center">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] text-orange-955 font-bold">جاري تحديث القاعدة...</span>
                        </div>
                      </div>
                    )}
                  </div>
        ...
        <div className="space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              ...
              <p className="text-[11px] text-stone-550 mt-1">مطالب استعادة الرسوم بموجب ميثاق وسياسة الإلغاء الشفافة لمنصة آثاري.</p>
            </div>

            <button
              onClick={() => setShowNewRefundModal(true)}
              className="bg-orange-700 hover:bg-orange-850 text-amber-50 font-black text-xs px-4  py-2.5 rounded-xl border-0 cursor-pointer shadow-sm flex items-center justify-center gap-2 shrink-0"
            >
          ...
          <div className="bg-white border border-stone-200 rounded-2xl overflow-hidden shadow-3xs">
                  ...
                  {refundRequests.map((req) => (
                    <tr key={req.id} className="hover:bg-stone-50/40 text-stone-700">
                      ...
                      <td className="p-4 whitespace-nowrap">
                        {req.status === 'Approved' && (
                          <div className="bg-emerald-50 text-emerald-850 border border-emerald-200 h-7 inline-flex items-center gap-1.5 px-3 rounded-full text-[10px] font-black">
                            <CheckCircle className="w-3.5 h-3.5 text-emerald-700" />
                            <span>مقبول • تمت التسوية</span>
                          </div>
                        )}
                        {req.status === 'Pending' && (
                          <div className="bg-amber-50 text-amber-850 border border-amber-200 h-7 inline-flex items-center gap-1.5 px-3 rounded-full text-[10px] font-black">
                            <Clock className="w-3.5 h-3.5 text-amber-700 animate-pulse" />
                            <span>قيد معالجة الشؤون</span>
                          </div>
                        )}
                        {req.status === 'Rejected' && (
                          <div className="bg-red-50 text-red-850 border border-red-200 h-7 inline-flex items-center gap-1.5 px-3 rounded-full text-[10px] font-black">
            ...
            <div className="p-4 bg-stone-50/50 border-t border-stone-150 flex items-center justify-between text-[11px] text-stone-500">
          ...
          <div className="bg-white rounded-3xl border border-amber-200 shadow-2xl relative max-w-lg w-full p-6 text-right z-10 animate-scale-up" dir="rtl">
            ...
            <form onSubmit={handleCreateRefundRequest} className="space-y-4">
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button
                  type="submit"
                  className="bg-orange-700 hover:bg-orange-850 text-white font-black px-6 py-2.5 rounded-xl text-xs transition border-0 cursor-pointer shadow-sm flex items-center justify-center gap-1.5"
                >
